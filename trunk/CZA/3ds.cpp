...
bool CLoad3DS::Import3DS(char *strFileName)
{
	...
	ProcessNextChunk(g_3DModel, &currentChunk);

	// After we have read the whole 3DS file, we want to calculate our own vertex normals.
	ComputeNormals(g_3DModel);
...
void CLoad3DS::init(char * pre_appened_path){
	...
	for(int i = 0; i < g_3DModel->numOfMaterials; i++)
	{
		...
		if(strlen(g_3DModel->pMaterials[i].strFile) > 0)
		{
			...
			string s1(pre_appened_path);
			string s2(g_3DModel->pMaterials[i].strFile);
			string s3(s1+s2);

			//combinedstr = strcat(pre_appened_path, g_3DModel->pMaterials[i].strFile);

			g_Texture[i] = CZAIL::BuildTexture(s3.c_str(), false);
